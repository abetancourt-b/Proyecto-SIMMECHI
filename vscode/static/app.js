    ...
    db.ref('historial_reciente').orderByKey().limitToLast(500).once('value').then(function (snapshot) {
        ...
        if (historialGuardado) {
            ...
            snapshot.forEach(function (childSnap) {
                registros.push(childSnap.val());
            });

            registros.forEach(function (registro, index) {
                if (registro && registro.timeStr) {
                    var row = document.createElement('tr');
                    // Cambio de logica: El indice 1 es el dato mas viejo de los 500, el indice 500 es el mas reciente
                    var indiceLegible = index + 1;
...
db.ref('sensores').on('value', function (snap) {
    ...
    // Si no hay timestamp se supone que deberia usar la hora actual de la pc
    if (!currentTimestamp || isNaN(new Date(currentTimestamp).getTime())) {
    ...
    // Solo guardar en historial si el timestamp es válido y genuino del sensor (no autogenerado)
    if (isValidTimestamp && currentTimestamp && String(currentTimestamp) !== String(lastSavedTimestamp)) {
